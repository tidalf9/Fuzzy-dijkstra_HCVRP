...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        # 特定问题的上下文参数（占位符和步骤上下文维度）
        if self.is_hcvrp:
            # Embedding of last node + remaining_capacity / remaining length / remaining prize to collect
            step_context_dim = embedding_dim + 1
            num_veh = 3
            node_dim = 2 + num_veh  # x,y, 需求（3辆车）
            node_veh = 9
        ...
        self.init_embed = nn.Linear(node_dim, embedding_dim)  # 节点嵌入
        ...
        # 为每个节点计算（glimpse key, glimpse value, logit key）共3个embedding_dim
