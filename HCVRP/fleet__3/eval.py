...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Evaluate the VRP model on a dataset.")
    parser.add_argument("datasets", nargs='+', help="Filename of the dataset(s) to evaluate.")
    parser.add_argument("-f", action='store_true', help="Overwrite the results file if it exists.")
    parser.add_argument("-o", help="Specify the name of the results file to write. Default is None.", default=None)
    parser.add_argument('--val_size', type=int, default=10000,
                        help='Number of instances used for reporting validation performance.')
    parser.add_argument('--offset', type=int, default=0,
                        help='Starting offset in the dataset (default is 0).')
    parser.add_argument('--eval_batch_size', type=int, default=1024,
                        help="Batch size to use during evaluation.")
    parser.add_argument('--width', type=int, nargs='+', default=[0],
                        help='Sizes of beam to use for beam search, or number of samples for sampling, '
                             '-1 for infinite. 0 to disable (default).')
    parser.add_argument('--decode_strategy', type=str, default='greedy',
                        help='Decoding strategy to use: "bs" for beam search, "sample" for sampling, or "greedy".')
    parser.add_argument('--softmax_temperature', type=parse_softmax_temperature, default=1,
                        help="Softmax temperature (used in sampling or beam search).")
    parser.add_argument('--model', type=str, required=True,
                        help='Path to the model file.')
    parser.add_argument('--no_cuda', action='store_true', help='Disable CUDA even if available.')
    parser.add_argument('--no_progress_bar', action='store_true', help='Disable the progress bar.')
    parser.add_argument('--compress_mask', action='store_true', help='Compress the mask into a long integer.')
    parser.add_argument('--max_calc_batch_size', type=int, default=10000000,
                        help='Maximum size for sub-batches during calculation.')
    parser.add_argument('--results_dir', default='results', help="Directory where results will be saved.")
    parser.add_argument('--obj', type=str, choices=['min-max', 'min-sum'], default='min-max',
                        help='Objective function configuration for the vehicle routing problem.')
    parser.add_argument('--multiprocessing', action='store_true',
                        help='Enable multiprocessing to parallelize over multiplec GPUs.')
