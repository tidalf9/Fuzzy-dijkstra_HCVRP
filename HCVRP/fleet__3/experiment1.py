    ...
    def forward(self, input):
        ...
        if isinstance(self.normalizer, nn.BatchNorm1d):
            return self.normalizer(input.view(-1, input.size(-1))).view(
                *input.size())  # [batch_size, graph_size+1, embed_dim]
...
# --- Pointer Network Module (if needed) ---
# (pointer_network content here if necessary)
...
class AttentionModel(nn.Module):
    ...
    def forward(self, input, return_pi=False):
        ...
        _log_p, log_p_veh, pi, veh_list, tour_1, tour_2, tour_3 = self._inner(input,
                                                                              embeddings)  # _log_p: [batch_size, graph_size+1, graph_size+1], pi:[batch_size, graph_size+1]

        cost, mask = self.problem.get_costs(input, self.obj, pi, veh_list, tour_1, tour_2,
                                            tour_3)  # mask is None, cost:[batch_size]
    ...
    def _inner(self, input, embeddings):
        ...
        while not (self.shrink_size is None and state.all_finished()):
            veh, log_p_veh = self.select_veh(input, state, sequences, embeddings, self.obj, veh_list, tour_1, tour_2,
                                             tour_3)  # [batch_size, 1]
            ...
            log_p, mask = self._get_log_p(fixed, state,
                                          veh)  # log_p: [batch_size, num_step, graph_size], mask:[batch_size, num_step, graph_size]

            # Select the indices of the next nodes in the sequences, result (batch_size) long
            selected = self._select_node(log_p.exp()[:, 0, :], mask[:, 0, :], state, veh,
    ...
    def sample_many(self, input, batch_rep=1, iter_rep=1):
        ...
        return sample_many(
            ...
            lambda input, pi, veh_list, tour_1, tour_2, tour_3: self.problem.get_costs(input[0], self.obj, pi, veh_list,
    ...
    def _get_log_p(self, fixed, state, veh, normalize=True):
        ...
        query = fixed.context_node_projected + \
                self.project_step_context(self._get_parallel_step_context(fixed.node_embeddings,
                                                                          state,
                                                                          veh))  # after project: [batch_size, 1, embed_dim]
            ...
            if from_depot:
                ...
                return torch.cat(  # [batch_size, num_veh, embed_dim+1]
                    (
                        ...
                        (torch.tensor(self.problem.VEHICLE_CAPACITY)[None, veh].cuda() - state.used_capacity[
                            torch.arange(batch_size), veh]).transpose(0, 1).unsqueeze(-1)
                    ),
                    -1
                )
    ...
    def _one_to_many_logits(self, query, glimpse_K, glimpse_V, logit_K, mask, veh):
        ...
        if self.mask_inner:  # True
            ...
            compatibility[mask[None, :, :, None, :].expand_as(
                compatibility)] = -math.inf  # nask visited nodes and nodes cannot be visited
...
# Function to initialize and load a pre-trained model
def load_attention_model(model_path, embedding_dim=128, hidden_dim=128, obj='objective', problem_name='tsp',
                         n_encode_layers=2):
    problem = type('Problem', (object,), {'NAME': problem_name})()
    model = AttentionModel(embedding_dim, hidden_dim, obj, problem, n_encode_layers)
    checkpoint = torch.load(model_path, map_location='cpu')  # Add map_location if necessary
