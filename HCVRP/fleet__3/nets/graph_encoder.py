...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, h=None, mask=None):
        """
        :param q: 查询 (batch_size, n_query, input_dim)
        :param h: 数据 (batch_size, graph_size, input_dim)
        :param mask: 掩码 (batch_size, n_query, graph_size) 或可视为此形状 (例如，如果 n_query == 1 则可以是2维)
        掩码应包含1表示无法进行注意力（即掩码是负邻接矩阵）
        :return:
        """
