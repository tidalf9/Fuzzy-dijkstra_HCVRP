...
def get_best(sequences, cost, veh_lists, ids=None, batch_size=None):
    """
    Ids 包含 [0, 0, 0, 1, 1, 2, ..., n, n, n]，表示第 0 个实例找到 3 个解，第 1 个实例找到 2 个解，以此类推
    :param sequences: 序列
    :param lengths: 长度
    :param ids: 标识
    :return: 包含 n 个序列的列表和包含解决方案长度的 n 个列表
    """
    if ids is None:
        idx = cost.argmin()
...
def eval_dataset(dataset_path, width, softmax_temp, opts):
    ...
    print("Average cost: {} +- {}".format(np.mean(final_cost), 2 * np.std(final_cost) / np.sqrt(len(final_cost))))
